...
class TensorboardLogger(Logger):
    ...
    def scalar_summary(self, tag, value, step):
        ...
        summary = tf_summary(value=[tf_summary.Value(tag=tag, simple_value=value)])
    ...
    def image_summary(self, tag, images, step):
        ...
        for i, img in enumerate(images):
            ...
            try:
                ...
                s = BytesIO()
            #scipy.misc.toimage(img).save(s, format="png") deprecato
            Image.fromarray(img).save(s, format="PNG")

            # Create an Image object
            img_sum = tf_summary.Image(encoded_image_string=s.getvalue(),
                                       height=img.shape[0],
                                       width=img.shape[1])
            # Create a Summary value
            img_summaries.append(tf_summary.Value(tag='%s/%d' % (tag, i), image=img_sum))

        # Create and write Summary
        summary = tf_summary(value=img_summaries)
        self.writer.add_summary(summary, step)

    def audio_summary(self, tag, sample, step, sr=16000):
        with tf.compat.v1.Session() as sess:
            audio_summary = tf.compat.v1.summary.audio(tag, sample, sample_rate=sr, max_outputs=4)
    ...
    def histo_summary(self, tag, values, step, bins=200):
        ...
        # SKIPPIAMO I VALORI NAN/INF
        if not np.isfinite(values).all():
            print(f"[warning] Skipping histogram for {tag} at step {step} because it contains inf or nan")
            return
        ...
        # Create and write Summary
        summary = tf_summary(value=[tf_summary.Value(tag=tag, histo=hist)])
        self.writer.add_summary(summary, step)
        self.writer.flush()

    def tensor_summary(self, tag, tensor, step):
        tf_tensor = tf.Variable(tensor).to_proto()
        summary = tf_summary(value=[tf_summary.Value(tag=tag, tensor=tf_tensor)])
